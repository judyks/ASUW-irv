...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const resultsByRoundButton = document.getElementById("resultsByRoundButton");

  fileInput.addEventListener("change", handleFileSelect);
  processButton.addEventListener("click", processButtonClick);
  document.getElementById("positionDropdown").addEventListener("change", updateLocationHash);
...
function toggleResultsDisplay() {
  ...
  document.getElementById("resultsByRoundButton").textContent = isHidden ? "Results by position" : "Results by Round";
}

function processData(csvData) {
  const lines = csvData.split("\n");
  const headers = lines[0].split(",");
  const positions = headers.map(header => header.split("presidential")[0].trim());
...
function initializePositionDropdown(uniquePositions) {
  ...
  positionDropdown.innerHTML = '<option value="">Select position</option>' + uniquePositions.map(position => `<option value="${position}">${position}</option>`).join("");
}
...
function processPositions(positions, lines, uniquePositions) {
  ...
  uniquePositions.forEach(position => {
    ...
    const { positionOutput, positionRoundResults } = createPositionOutput(position, lines);
    ...
    positionRoundResults.forEach((roundResult, roundIndex) => {
      ...
      roundResults[roundIndex].push(`<h4>Position: ${position}</h4>${roundResult}`);
    });
  });
...
function isAllowedPosition(position) {
  ...
  return allowedHeaders.some(header => position.toLowerCase().startsWith(header.toLowerCase()));
}
...
function createPositionOutput(position, lines) {
  ...
  while (!winnerDeclared) {
    ...
    let { newEliminatedCandidates, eliminatedThisRound, isTie } = updateEliminatedCandidates(sortedVotes, eliminatedCandidates, totalVotes);

    let roundOutput = generateRoundOutput(round, sortedVotes, totalVotes, eliminatedThisRound, eliminatedCandidates);
    output += roundOutput;
...
function getPositionVotes(position, lines) {
  return lines.slice(1).map(line => line.split(",").filter((_, index) => lines[0].split(",")[index].trim().startsWith(position)).map(pref => pref.trim()).filter(pref => pref)).filter(vote => vote.length > 0);
}
...
function generateRoundOutput(round, sortedVotes, totalVotes, eliminatedThisRound, allEliminatedCandidates) {
  ...
  if (eliminatedThisRound.length > 0) {
    output += `<p>Eliminated this round: ${eliminatedThisRound.join(", ")}</p>`;
  }
  if (allEliminatedCandidates.size > 0) {
    output += `<p>Eliminated candidates so far: ${Array.from(allEliminatedCandidates).join(", ")}</p>`;
  }
...
function declareWinner(sortedVotes, totalVotes, position) {
  ...
  winners[position] = { name: winner, votes: maxVotes, percentage: winnerPercentage };
  return `<h4>Winner: ${winner} with ${maxVotes} votes (${winnerPercentage}% of total votes)</h4>`;
}

function updateEliminatedCandidates(sortedVotes, eliminatedCandidates, totalVotes) {
  let newEliminatedCandidates = new Set(eliminatedCandidates);
  let minVotes = sortedVotes[sortedVotes.length - 1][1];
  let candidatesForElimination = sortedVotes.filter(([_, votes]) => votes === minVotes).map(([candidate]) => candidate);
  let isTie = sortedVotes.length === 2 && sortedVotes[0][1] === sortedVotes[1][1];

  if (!isTie) {
    candidatesForElimination.forEach(candidate => newEliminatedCandidates.add(candidate));
  }

  return { newEliminatedCandidates, eliminatedThisRound: isTie ? [] : candidatesForElimination, isTie };
}
...
function updateWinnersOverview() {
  ...
  overviewHtml += "<tr><th>Position</th><th>Winner</th><th>Votes</th><th>Percentage</th></tr>";
...
function displayRoundResults(roundResults) {
  ...
  Object.keys(roundResults).forEach(round => {
    roundResultsOutput += `<h2>Round ${parseInt(round) + 1}</h2>` + roundResults[round].join("");
  });
  document.getElementById("roundResultsContainer").innerHTML = roundResultsOutput;
}
